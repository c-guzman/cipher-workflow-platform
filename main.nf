 ...
 */
 ...
 if (params.mode == 'chip' && params.lib == 's') {
 	...
 	process create_coverage_tracks_WI {
 		...
 		script:
 		"""
 		bamCompare -b1 ${bam} -b2 ${control} -o ${id}.minusInput.RPGCnorm.bigWig -of bigwig --ratio subtract -bs 10 -p ${params.threads} --normalizeTo1x ${egs_size} --smoothLength 60 -e ${fragLen} --ignoreDuplicates --centerReads
 		"""
 	}
